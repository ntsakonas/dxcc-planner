...
public class DXCCPlanner
{
	...
	DXCCEntitiesReader countries = new DXCCEntitiesReader(COUNTRY_FILE,MOST_WANTED_FILE);
	
	
	copyDXCCList(countries.getDXCCList());
	...
	calculateBeamAndDistancesOfAllDXCCFromMe();
    ...
    private void copyDXCCList(ArrayList<DXCCEntity> dxccList)
    {
	for (DXCCEntity entity :dxccList)
	    _dxccList.add(entity);
    }
    ...
    // calculate and update all DXCC entries on the DXCC list with the distance/bearing (in Km/degrees) from the central location
    private void calculateBeamAndDistancesOfAllDXCCFromMe()
    {
